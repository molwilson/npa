...
```{r}
fish <- read_excel(here("reef monitoring", "data", "npa_ltm_test.xlsx"), sheet = "Fish Data Entry") %>% # set this to location of data file or import manually
  clean_names() %>%
...
fish_density_tot <- fish %>%
  ...
  summarize(count_tot = sum(count),
            density = count_tot / fish_belt_area) %>%
