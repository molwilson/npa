...
```{r}
fish <- read_excel(here("reef monitoring", "data", "npa_ltm_test.xlsx"), sheet = "Fish Data Entry") %>%  
  clean_names() %>%
...
fish_col <- geom_col(fill = "cadetblue", color = "black", alpha = 0.5)
```
...
ggplot(fish_density_grp_size %>% filter(site == "Pillars"), aes(x = size_bin, y = density_mean)) +
  fish_col +
  geom_errorbar(aes(ymin = density_mean - density_se, ymax = density_mean + density_se), width = .2,
                 position = position_dodge(.9)) +
...
#### Mermaids
```{r, fig.width = 12}
ggplot(fish_density_grp_size %>% filter(site == "Mermaids"), aes(x = size_bin, y = density_mean)) +
  fish_col +
  geom_errorbar(aes(ymin = density_mean - density_se, ymax = density_mean + density_se), width = .2,
                 position = position_dodge(.9)) +
  facet_grid(year ~ group) +
