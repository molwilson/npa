...
* This includes only fish - can change if we want to include the "other" section (turtles/sharks/etc.)
```{r}
...
ggplot(fish_density_size %>% filter(site == "Pillars of Hercules"), aes(x = size_bin, y = density_mean)) +
  geom_col() +
  geom_errorbar(aes(ymin = density_mean - density_se, ymax = density_mean + density_se), width = .2,
                 position = position_dodge(.9)) +
...
ggplot(benthic_heights %>%
         mutate(algal_type = to_any_case(algal_type, case = "title")), aes(x = algal_type, y = height_mean)) +
  geom_col() +
